...
namespace NinetiesTV
{
    class Program
    {
        ...
        **************************************************************************************************/
        ...
        {
            return shows.OrderByDescending(show => show.ImdbRating).SkipLast(1).ToList();
        }
        ...
        {
            return shows.OrderBy(show => show.EndYear).Where(show => show.EndYear >= 2000).ToList()[0];
        }
        ...
        {
            return shows.OrderByDescending(show => show.ImdbRating).First(show => show.Genres.Contains("Drama"));
        }

        // 18. Return all dramas except for the highest rated.
        static List<Show> AllButBestDrama(List<Show> shows)
        {
            return shows.OrderByDescending(show => show.ImdbRating).Where(show => show.Genres.Contains("Drama")).Skip(1).ToList();
        }

        // 19. Return the number of crime shows with an IMDB rating greater than 7.0.
        static int GoodCrimeShows(List<Show> shows)
        {
            return shows.Where(show => show.Genres.Contains("Crime") && show.ImdbRating > 7.0).ToList().Count;
        }
        ...
        {
            return shows
            .Where(show => show.ImdbRating < 8.0 && show.EndYear - show.StartYear > 10)
            .OrderBy(show => show.Name).ToList()[0];
        }

        // 21. Return the show with the most words in the name.
        // static Show WordieastName(List<Show> shows)
        static List<Show> WordieastName(List<Show> shows)
        {
            return shows.OrderByDescending(show => show.Name.Split(' ').Length).ToList();
        }
        ...
        {
            return String.Join(", ", shows.Select(show => show.Name).SkipLast(1)) + ", and " + shows.Select(show => show.Name).Last();
        }
